...
embedder2 = AutoModel.from_pretrained('BM-K/KoSimCSE-roberta-multitask')
embedder2_tok = AutoTokenizer.from_pretrained('BM-K/KoSimCSE-roberta-multitask')

def get_context(question : str, kD : dict) : 
    global category
    corpus = list(kD.keys())
    hits = util.semantic_search(embedder.encode(question),embedder.encode(corpus))      #가장유사한 문서를 반환 (수성못, 팔공산...중에서)
    print("hit의 내용 출력: ", hits)
    print("---------------------------")

    #유사도가 0.5이상이면 hits내용(카테고리), 이하면 빈 문자열 반환
    category = corpus[hits[0][0]['corpus_id']] if hits[0][0]['score'] > 0.5 else ''
    return corpus[hits[0][0]['corpus_id']] if hits[0][0]['score'] > 0.5 else ''

def get_fitting_mod(data,question,size,tokenizer,hist,prev) :
    
    #context_embedding=embedder2(**embedder2_tok(data['context'].split('\n\n'),return_tensors='pt',padding=True,truncation=True),return_dict=False)
    #quest_embedding=embedder2(**embedder2_tok(data['question'],return_tensors='pt',padding=True,truncation=True),return_dict=False)
    #hits=util.semantic_search(quest_embedding,context_embedding)
    ...
    x = PROMPT_DICT['prompt_input'].format(instruction=question, input=data['context']+hist)    #input='\n\n'.join(data['context'])+hist : context한글자씩 띄우고 hist내용더하기
    print("x의 내용 출력: ",x)
    print("prev의 내용 출력:\n",prev)
    print("hist의 내용 출력: \n",hist)
...
def get_answer(data,question) :
    ...
    #quest_embedding=embedder2(**embedder2_tok(data['question'],return_tensors='pt',padding=True,truncation=True),return_dict=False)
    #hits=util.semantic_search(quest_embedding,context_embedding)
    hits=util.semantic_search(embedder.encode(question),embedder.encode(data['question']))   #data['question']에서 입력질문과 가장 비슷한 거 반환
    l = len(hits[0])
        ...
        if x[1]>0.5 :           #반환된 답의 정확도가 0.5이상일 경우
            ...
            if context != '' :
                ...
                if prev_quest=='':
                    ...
                    continue
                da=get_fitting_mod(kd[context],user_input,2048-128,tokenizer,hist,prev_quest)
                print("da 내용 : ",da)
...
if __name__=='__main__' :
    ...
    model = LlamaForCausalLM.from_pretrained(config.base_model_name_or_path,device_map='auto',load_in_8bit=True)
